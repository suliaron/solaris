...
__host__ __device__ vec_t	circular_velocity(var_t mu, var_t r, var_t alpha);
__host__ __device__ vec_t	gas_velocity(var2_t eta, var_t mu, var_t r, var_t alpha);
__host__ __device__ var_t	gas_density_at(const planets::gaspar_t* gaspar, var_t r, var_t z);
__host__ __device__ var_t	calculate_kinetic_energy(const vec_t* velo);
__host__ __device__ var_t	calculate_potential_energy(var_t mu, const vec_t* coor);
__host__ __device__ var_t	calculate_energy(var_t mu, const vec_t* coor, const vec_t* velo);
__host__ __device__ int_t	kepler_equation_solver(var_t ecc, var_t mean, var_t eps, var_t* E);
__host__ __device__ int_t	calculate_phase(var_t mu, const planets::orbelem_t* oe, vec_t* coor, vec_t* velo);
